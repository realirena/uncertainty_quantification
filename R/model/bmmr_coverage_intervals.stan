...
data {
  ...
  matrix[S, X] phi; // sex-specific means age-distributions - this model assumes they are inputs, but we can code a model for the age groups 
  matrix[S,X] sigma; // lower bounds for the age distribution estimates 
...
model {
 pr_raw ~ beta(2,2); // reporting error multiplier 
  ...
  for(s in 1:S){
      for(x in 1:X){
        theta_x[s,x] ~ normal(phi[s,x], sigma[s,x]); // priors on the age distributions - the model works without this, but this could also help w/ identifiability (see Schmertmann 2018)
      }
  }
