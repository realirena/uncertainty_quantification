...
##--------------------------------------------------##
##
## LC forecasts for Palestine project
## 
##--------------------------------------------------##
...
## loading data
data <- readRDS("master_regional_deaths_population_age_sex_2012_2023.rds")
...
data.with.fore %>% 
  ...
  filter(!(source=="wpp2024" & year == 2023)) %>% 
  # filter(source=="lc") %>% 
...
ggsave("e0fore.pdf",width = 12,height=8)  

## saving data
saveRDS(data.with.fore, file = "data_plus_forecasts.rds")
